...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        var = forward(model, inputs_valid)
        prediction = Softmax(var['y'])        
        plot_uncertain_images(inputs_valid, target_valid,prediction)
...
def plot_uncertain_images(x, t, prediction, threshold=0.4):
    """ Plot the image if network has no confident in classifying it """
